...
INPUT_DATA MySQLService::getInputData(uint16_t years, const char *PRODUCT_SERIAL_NUMBER, const char *DATE_TIME){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){strcpy(output.product_serial_number, rows[0]);}
		if(rows[1] != nullptr){strcpy(output.date_time, rows[1]);}
		if(rows[2] != nullptr){output.temp = atof(rows[2]);}
		if(rows[3] != nullptr){output.hmdty = atof(rows[3]);}
		if(rows[4] != nullptr){output.pm25 = atof(rows[4]);}
		if(rows[5] != nullptr){output.pm10 = atof(rows[5]);}
		if(rows[6] != nullptr){strcpy(output.mvmnt, rows[6]);}
		if(rows[7] != nullptr){output.tvoc = atof(rows[7]);}
		if(rows[8] != nullptr){output.hcho = atof(rows[8]);}
		if(rows[9] != nullptr){output.co2 = atof(rows[9]);}
		if(rows[10] != nullptr){output.co = atof(rows[10]);}
		if(rows[11] != nullptr){output.benzo = atof(rows[11]);}
		if(rows[12] != nullptr){output.radon = atof(rows[12]);}
		if(rows[13] != nullptr){strcpy(output.mod_date, rows[13]);}
		if(rows[14] != nullptr){strcpy(output.reg_date, rows[14]);}
		if(rows[15] != nullptr){output.tmp = atof(rows[15]);}
		if(rows[16] != nullptr){output.tmp2 = atof(rows[16]);}
		if(rows[17] != nullptr){output.tmp3 = atof(rows[17]);}
		if(rows[18] != nullptr){output.tmp4 = atof(rows[18]);}
		if(rows[19] != nullptr){output.tmp5 = atof(rows[19]);}
		if(rows[20] != nullptr){strcpy(output.tmp6, rows[20]);}
		if(rows[21] != nullptr){strcpy(output.tmp7, rows[21]);}
		if(rows[22] != nullptr){strcpy(output.tmp8, rows[22]);}
		if(rows[23] != nullptr){strcpy(output.tmp9, rows[23]);}
		if(rows[24] != nullptr){strcpy(output.tmp10, rows[24]);}
	}
...
IOT_PRDCT_AUTH MySQLService::getIotPrdctAuth(const char *serial_number){
	...
	while(rows = mysql_fetch_row(res)){
		if(rows[0] != nullptr){strcpy(iot_prdct_auth.product_serial_number, rows[0]);}
		if(rows[1] != nullptr){iot_prdct_auth.auth_yn = strcmp(rows[1], "Y")? 0:1;}
		if(rows[2] != nullptr){iot_prdct_auth.use_yn = strcmp(rows[2], "Y")? 0:1;}
		if(rows[3] != nullptr){strcpy(iot_prdct_auth.mod_date, rows[3]);}
		if(rows[4] != nullptr){strcpy(iot_prdct_auth.reg_date, rows[4]);}
	}
...
KEPCO_API_AUTH MySQLService::getKepcoApiAuth(const char *api_key){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){memcpy(kepco_api_auth.kepco_api_key, rows[0], strlen(rows[0]));}
		if(rows[1] != nullptr){kepco_api_auth.auth_yn = strcmp(rows[1], "Y")? 0:1;}
		if(rows[2] != nullptr){kepco_api_auth.use_yn = strcmp(rows[2], "Y")? 0:1;}
		if(rows[3] != nullptr){memcpy(kepco_api_auth.mod_date, rows[3], strlen(rows[3]));}
		if(rows[4] != nullptr){memcpy(kepco_api_auth.reg_date, rows[4], strlen(rows[4]));}
	}
	mysql_free_result(result);
...
MEMBER MySQLService::getMember(uint32_t CUSTOMER_LINK_NUMBER, const char *PRODUCT_SERIAL_NUMBER){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){strcpy(output.product_serial_number, rows[0]);}
		if(rows[1] != nullptr){output.customer_link_number = atoi(rows[1]);}
		if(rows[2] != nullptr){strcpy(output.kepco_api_key, rows[2]);}
		if(rows[3] != nullptr){strcpy(output.sns_id, rows[3]);}
		if(rows[4] != nullptr){strcpy(output.sns_pwd, rows[4]);}
		if(rows[5] != nullptr){strcpy(output.sns_key, rows[5]);}
		if(rows[6] != nullptr){strcpy(output.sns_type, rows[6]);}
		if(rows[7] != nullptr){strcpy(output.user_name, rows[7]);}
		if(rows[8] != nullptr){strcpy(output.phone_number, rows[8]);}
		if(rows[9] != nullptr){strcpy(output.zip_number, rows[9]);}
		if(rows[10] != nullptr){strcpy(output.addr, rows[10]);}
		if(rows[11] != nullptr){strcpy(output.addr_dtl, rows[11]);}
		if(rows[12] != nullptr){output.lon = atof(rows[12]);}
		if(rows[13] != nullptr){output.lat = atof(rows[13]);}
		if(rows[14] != nullptr){output.house_area = atof(rows[14]);}
		if(rows[15] != nullptr){output.male_cnt = atoi(rows[15]);}
		if(rows[16] != nullptr){output.female_cnt = atoi(rows[16]);}
		if(rows[17] != nullptr){output.room_cnt = atoi(rows[17]);}
		if(rows[18] != nullptr){output.tv_cnt = atoi(rows[18]);}
		if(rows[19] != nullptr){output.air_conditioner_cnt = atoi(rows[19]);}
		if(rows[20] != nullptr){output.washing_machine_cnt = atoi(rows[20]);}
		if(rows[21] != nullptr){output.dryer_cnt = atoi(rows[21]);}
		if(rows[22] != nullptr){output.rice_cooker_cnt = atoi(rows[22]);}
		if(rows[23] != nullptr){output.fan_cnt = atoi(rows[23]);}
		if(rows[24] != nullptr){output.disk_washer_cnt = atoi(rows[24]);}
		if(rows[25] != nullptr){output.computer_cnt = atoi(rows[25]);}
		if(rows[26] != nullptr){strcpy(output.rate_plan, rows[26]);}
		if(rows[27] != nullptr){output.use_yn = strcmp(rows[27], "Y")? 0:1;}
		if(rows[28] != nullptr){strcpy(output.mod_date, rows[28]);}
		if(rows[29] != nullptr){strcpy(output.reg_date, rows[29]);}
	}
	mysql_free_result(result);
...
MSG_INFO MySQLService::getMsgInfo(const char *msg_code){
	...
	memset(&output, 0x00, sizeof(output));
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){strcpy(output.msg_code, rows[0]);}
		if(rows[1] != nullptr){strcpy(output.msg_version, rows[1]);}
		if(rows[2] != nullptr){strcpy(output.msg_cont, rows[2]);}
		if(rows[3] != nullptr){output.use_yn = strcmp(rows[3], "Y")? 0:1;}
		if(rows[4] != nullptr){strcpy(output.mod_date, rows[4]);}
		if(rows[5] != nullptr){strcpy(output.reg_date, rows[5]);}
	}
	mysql_free_result(result);
...
OUTPUT_DATA MySQLService::getOutputData(uint16_t years, uint32_t CUSTOMER_LINK_NUMBER, const char *dateTime){
	...
	memset(&output, 0x00, sizeof(output));
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){output.customer_link_number = atoi(rows[0]);}
		if(rows[1] != nullptr){memcpy(output.date_time, rows[1], strlen(rows[1]));}
		if(rows[2] != nullptr){output.temp = atof(rows[2]);}
		if(rows[3] != nullptr){output.hmdty = atof(rows[3]);}
		if(rows[4] != nullptr){output.pm25 = atof(rows[4]);}
		if(rows[5] != nullptr){output.pm10 = atof(rows[5]);}
		if(rows[6] != nullptr){output.dust = atof(rows[6]);}
		if(rows[7] != nullptr){output.co2 = atof(rows[7]);}
		if(rows[8] != nullptr){output.pwr = atof(rows[8]);}
		if(rows[9] != nullptr){output.gas = atof(rows[9]);}
		if(rows[10] != nullptr){output.water = atof(rows[10]);}
		if(rows[11] != nullptr){output.prd_pwr = atof(rows[11]);}
		if(rows[12] != nullptr){output.prd_gas = atof(rows[12]);}
		if(rows[13] != nullptr){output.prd_water = atof(rows[13]);}
		if(rows[14] != nullptr){memcpy(output.mod_date, rows[14], strlen(rows[14]));}
		if(rows[15] != nullptr){memcpy(output.reg_date, rows[15], strlen(rows[15]));}
	}
	mysql_free_result(result);
...
OUTPUT_STAT_DATA MySQLService::getOutputStatData(uint16_t years, uint32_t CUSTOMER_LINK_NUMBER, const char *dateTime){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){output.customer_link_number = atoi(rows[0]);}
		if(rows[1] != nullptr){memcpy(output.date_time, rows[1], strlen(rows[1]));}
		if(rows[2] != nullptr){output.prv_pwr = atof(rows[2]);}
		if(rows[3] != nullptr){output.prv_gas = atof(rows[3]);}
		if(rows[4] != nullptr){output.prv_water = atof(rows[4]);}
		if(rows[5] != nullptr){output.prd_prv_pwr = atof(rows[5]);}
		if(rows[6] != nullptr){output.prd_prv_gas = atof(rows[6]);}
		if(rows[7] != nullptr){output.prd_prv_water = atof(rows[7]);}
		if(rows[8] != nullptr){output.pre_pwr = atof(rows[8]);}
		if(rows[9] != nullptr){output.pre_gas = atof(rows[9]);}
		if(rows[10] != nullptr){output.pre_water = atof(rows[10]);}
		if(rows[11] != nullptr){output.prd_pre_pwr = atof(rows[11]);}
		if(rows[12] != nullptr){output.prd_pre_gas = atof(rows[12]);}
		if(rows[13] != nullptr){output.prd_pre_water = atof(rows[13]);}
		if(rows[14] != nullptr){memcpy(output.mod_date, rows[14], strlen(rows[14]));}
		if(rows[15] != nullptr){memcpy(output.reg_date, rows[15], strlen(rows[15]));}
		counter++;
	}
...
SYS_INFO MySQLService::getSysInfo(uint32_t CUSTOMER_LINK_NUMBER){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){output.customer_link_number = atoi(rows[0]);}
		if(rows[1] != nullptr){output.pwr_svn_trg = atoi(rows[1]);}
		if(rows[2] != nullptr){output.gas_svn_trg = atoi(rows[2]);}
		if(rows[3] != nullptr){output.wtr_svn_trg = atoi(rows[3]);}
		if(rows[4] != nullptr){output.alarm_use_yn = strcmp(rows[4], "Y")? 0:1;}
		if(rows[5] != nullptr){output.alarm_sound_yn = strcmp(rows[5], "Y")? 0:1;}
		if(rows[6] != nullptr){output.alarm_vbrtn_yn = strcmp(rows[6], "Y")? 0:1;}
		if(rows[7] != nullptr){output.alarm_optm_yn = strcmp(rows[7], "Y")? 0:1;}
		if(rows[8] != nullptr){output.use_yn = strcmp(rows[8], "Y")? 0:1;}
		if(rows[9] != nullptr){strcpy(output.reg_date, rows[9]);}
		if(rows[10] != nullptr){strcpy(output.mod_date, rows[10]);}
		if(rows[11] != nullptr){output.tmp = atof(rows[11]);}
		if(rows[12] != nullptr){output.tmp2 = atof(rows[12]);}
		if(rows[13] != nullptr){output.tmp3 = atof(rows[13]);}
		if(rows[14] != nullptr){output.tmp4 = atof(rows[14]);}
		if(rows[15] != nullptr){output.tmp5 = atof(rows[15]);}
		if(rows[16] != nullptr){strcpy(output.tmp6, rows[16]);}
		if(rows[17] != nullptr){strcpy(output.tmp7, rows[17]);}
		if(rows[18] != nullptr){strcpy(output.tmp8, rows[18]);}
		if(rows[19] != nullptr){strcpy(output.tmp9, rows[19]);}
		if(rows[20] != nullptr){strcpy(output.tmp10, rows[20]);}
		counter++;
	}
	mysql_free_result(result);
...
TERMS_COND MySQLService::getTermsCond(uint32_t CUSTOMER_LINK_NUMBER){
	...
	while(rows = mysql_fetch_row(result)){
		if(rows[0] != nullptr){output.customer_link_number = atoi(rows[0]);}
		if(rows[1] != nullptr){strcpy(output.terms_code, rows[1]);}
		if(rows[2] != nullptr){strcpy(output.terms_version, rows[2]);}
		if(rows[3] != nullptr){strcpy(output.terms_cont, rows[3]);}
		if(rows[4] != nullptr){strcpy(output.agrtr_cndtn_yn, rows[4]);}
		if(rows[5] != nullptr){output.use_yn = strcmp(rows[5], "Y")? 0:1;}
		if(rows[6] != nullptr){strcpy(output.reg_date, rows[6]);}
		if(rows[7] != nullptr){strcpy(output.mod_date, rows[7]);}
		counter++;
	}
	mysql_free_result(result);
